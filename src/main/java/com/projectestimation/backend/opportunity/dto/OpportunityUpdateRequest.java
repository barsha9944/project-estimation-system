...
public record OpportunityUpdateRequest(
		...
		@Size(max = 5000, message = "Requirement summary must not exceed 5000 characters") String requirementSummary,
		@NotNull(message = "Priority is required") Priority priority, LocalDate expectedDeliveryDate,
		@NotNull(message = "Components are required") List<String> components, OpportunityStatus status,
		FileStorageDto opportunityFileStorage) {
}
